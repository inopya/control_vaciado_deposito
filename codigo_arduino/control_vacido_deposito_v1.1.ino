...
{ 
  ...
  if(segundos!=segundos_anterior){
    ...
    switch (estado_actual) 
    {
      ...
      case MQ_OPENNIG:
        lcd.setCursor(0,0);
        mostrar_hora(); 
        //parpadeo_led();                                         // mostrar parpadeo mientras se mueve la valvula
        lcd.setCursor(0,1);
  ...
  if( pulsacion==PULSACION_LARGA && estado_actual==MQ_WAITING ){
    segundos_anterior--;
  ...
  if ( pulsacion==PULSACION_LARGA && estado_actual == MQ_EMPTYING ){
    segundos_anterior--;
  ...
  if( pulsacion==PULSACION_DOBLE && estado_actual==MQ_WAITING ){
    ...
    segundos_anterior--;       // para provocar el refresco del display tras salir de la programcion
    FLAG_timeout_running = true;
    date = rtc.now();
    timer_luz_lcd = date.unixtime() + TIMEOUT;
    pulsacion=PULSACION_NULA;
